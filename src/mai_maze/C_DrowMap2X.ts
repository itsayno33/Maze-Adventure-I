...
export class C_DrowMap2X {
    static me: C_DrowMap2X|undefined= undefined;

    static newObj(a: T_DrowMap2X): C_DrowMap2X {
        C_DrowMap2X.me ??= new C_DrowMap2X(a);
        return C_DrowMap2X.me;
    }
...
****************/
    ...
    protected constructor(a: T_DrowMap2X) {
        ...
        this.set_xy({col: a.col, row: a.row, x_min: a.x_min, y_min: a.y_min} as T_DrowMap2X_Opt);
    }

    public set_xy(a: T_DrowMap2X_Opt): void {
        if (a.col   < 3 || a.row   < 3) return;
        if (a.x_min < 3 || a.y_min < 3) return;

        if (this.div === null || this.cvs === null) return;
        this.c_min_x = a.x_min;
        this.c_min_y = a.y_min; 
        this._calc_map_size(a.col, a.row);
    }
    ...
    protected _calc_map_size(col: number, row: number): void {
        ...
        this.c_size_x = _max([this.c_min_x, _round(1.00 *  _min([col_px, row_px]), 2)]);
        this.c_size_y = _max([this.c_min_y, _round(1.00 *  _min([col_px, row_px]), 2)]);

        this.map_wdth     = this.c_size_x * col;
        this.map_hght     = this.c_size_y * col;

        this.cvs.setAttribute('width',  this.map_wdth.toString());
        this.cvs.setAttribute('height', this.map_hght.toString());
    }

    public calc_map_top(pd: C_PointDir): void {

        this.view_wdth  = this.div.clientWidth;
        this.view_hght  = this.div.clientHeight;

        let top_x =  this.view_wdth / 2 - pd.x * this.c_size_x;
        if (top_x > 0) top_x = 0; // 左端制限
        if (top_x < this.view_wdth - this.map_wdth) top_x = this.view_wdth - this.map_wdth; // 右端制限

        let top_y =  this.view_hght / 2 - pd.y * this.c_size_y;
    ...
    //        alert(`View2D: ${this.view_wdth}x${this.view_hght} px, Cell: ${this.c_size_x}x${this.c_size_y} px`);
    //        alert(`View2D: ${top_x}px * ${top_y}px, PD: ${pd.x},${pd.y},${pd.z}`);
    }

    public drow_map2X(pd: C_PointDir, maze: C_Maze, is_masked: boolean = true): void { 
        if (this.cvs !== null) {this.to_2X(pd, maze, is_masked);this.calc_map_top(pd)};
    }


    public to_2X(pd: C_PointDir, maze: C_Maze, is_masked: boolean  = true): void {
        const size_x = maze.get_x_max();
        const size_y = maze.get_y_max();
        const floor  = pd.z

        for (let y = 0; y < size_y; y++) {
            for (let x = 0; x < size_x; x++) {
                let rect_2d: T_Rect = {
                    tl: { x: ( x     ) * this.c_size_x, y: ( y     ) * this.c_size_y },
                    tr: { x: ( x + 1 ) * this.c_size_x, y: ( y     ) * this.c_size_y },
                    dl: { x: ( x     ) * this.c_size_x, y: ( y + 1 ) * this.c_size_y },
                    dr: { x: ( x + 1 ) * this.c_size_x, y: ( y + 1 ) * this.c_size_y },
                };

                if (!is_masked && maze.is_masked_xyz(x, y, floor)) {
                    // マスクされているセルは、マスク表示
