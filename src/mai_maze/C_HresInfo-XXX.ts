...
export class C_HresInfo_XXX {
    ...
    protected _createTable(): void {
        ...
        const tbl_id = this.elm.id + '_table';
        this.tbl = document.getElementById(tbl_id) as HTMLTableElement;
        if (this.tbl === undefined || this.tbl === null) return;

        this.tbl.setAttribute('display', 'none');

        const bdy_id = this.elm.id + '_tbody';
        const tbody = document.getElementById(bdy_id);
        if (tbody === undefined || tbody === null) return;
